...
class TimeValue(Layer, LabelConversions, layer=Layer.Type.InputOutput):
    ...
    def _decode(self, rawstream: pd.DataFrame) -> pd.DataFrame:
        ...
        for matcher, operation in self.config.matcher:
            ...
            if operation == "max":
                tmp_data = pd.DataFrame({_col_name:rawstream[matcher].iloc[:, 1:].max(axis=1).copy()})
            elif operation == "min":
                tmp_data = pd.DataFrame({_col_name:rawstream[matcher].iloc[:, 1:].min(axis=1).copy()})
            elif operation == "sum":
                tmp_data = pd.DataFrame({_col_name:rawstream[matcher].iloc[:, 1:].sum(axis=1).copy()})
            else:
                tmp_data = rawstream[matcher].iloc[:, 1:].copy()
            for columnname in tmp_data.columns:
                rdpstream.insert(loc=len(rdpstream.columns), column=columnname, value=tmp_data[columnname])
        try:
