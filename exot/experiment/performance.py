    ...
    def digest(self, *, skip_checks: bool = False, **kwargs) -> None:
        ...
        if isinstance(self.config.bit_count, list):
            if len(self.config.bit_count) != 2:
                raise ValueError("if bit_count is a list, it has to have only two values [min, max] for the random range")
            self.config.bit_count = np.random.randint(self.config.bit_count[0], self.config.bit_count[1]+1, 1, dtype=np.dtype("int"))
