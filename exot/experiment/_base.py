...
from exot.util.mixins import (
    Configurable,
    HasParent,
    IntermediatesCollector,
    Pickleable,
    SubclassTracker,
)
from exot.util.prompts import prompt
from exot.util.timeout import Timeout
from exot.util.wrangle import app_log_formatter, generic_log_formatter, run_path_unformatter
...
class Experiment(
    ...
    def _validate_environment(env: t.Mapping) -> t.NoReturn:
        ...
        for var in env.values():
            ...
            if var["driver_type"] == "SSHUnixDriver":
                # connection
        ...
        for env in self.config.ENVIRONMENTS:
            for app in self.config.ENVIRONMENTS[env]["APPS"]:
                ...
                if _type == "standalone":
                    ...
                    if app not in self.config.ENVIRONMENTS[env]:
                        those_that_have = [
                            _ for _ in self.config.ENVIRONMENTS[env] if _ != "APPS"
                        ]
                        raise MisconfiguredError(
                    ...
                    app_config = self.config.ENVIRONMENTS[env][app]
                    sched = _app.get("sched", None)
                    sched = Path(sched).name if sched else sched
                    is_standalone = False
    ...
    def write(self) -> None:
        ...
        def move_action(path):
            m_path = add_timestamp(path, "modified")
            if m_path.exists():
                m_path = add_random(m_path)
            self.logger.info(
                f"experiment path '{path}' already exists, will be moved to '{m_path}'"
            )
            move(path, m_path)

        self._update_mode: bool = self.config.experiment_exists_action == "update"
    ...
    @classmethod
    def read(
        cls, path: Path, new_root_path: t.Optional[Path] = None, *, replace: bool = True
    ) -> Experiment:
        ...
        Args:
            ...
            replace (bool, optional): Replace values from the instance with those from files?
        ...
        if contained_environments.exists():
    ...
    def print_duration(self) -> t.Dict:
    ...
    def _get_execution_status(
        ...
        if not env:
            ...
            if query:
                query_result = getitem(self._execution_status, query)
                index_of_env = query_result.index(next(_ for _ in query_result if _[0] == env))
                return getitem(self._execution_status, query + (index_of_env,))[1]
            else:
    ...
    def _update_execution_status(self) -> None:
        ...
        """
        self._execution_status = map_to_leaves(
            lambda run: [
                (env, list(v.values())) for env, v in run.executed_repetitions.items()
            ],
            self.phases,
            _seq=False,
        )
    ...
    def _modify_execution_status(
        ...
        setitem(self._execution_status, query + (index_of_env,), (env, value))
    ...
    def _push_execution_status(self) -> None:
        ...
        """
        for stats, run in zip(leaves(self._execution_status), leaves(self.phases)):
            for env, rep_stats in stats:
                run._executed_repetitions[env] = {i: v for i, v in enumerate(rep_stats)}

    def execute_in_environment(self, env: str, phases: t.List[str], resume=False) -> None:
        ...
        runs = (
            ...
            (
                ...
                if not all(list(run.executed_repetitions[env].values()))
            )
            if resume
        ...
        for zone, driver in self.drivers[env].items():
            driver.connect()  # gets original settings

            self.logger.info(f"{env}->{zone}: connected: {driver.__class__.__name__}")
            self.logger.debug(f"{env}->{zone}: original state: {driver.original_state!r}")

            self.logger.info(f"{env}->{zone}: sending experiment")
            if driver.exists(str(self.remote_path(env, zone))):
                self.logger.debug(
                    f"{env}->{zone}: remote experiment directory exists, will be deleted"
                )
                _ = driver.delete(str(self.remote_path(env, zone)), recursive=True)
                if _:
                    self.logger.debug(
                        f"{env}->{zone}: deleted successfully: {self.remote_path(env, zone)!s}"
                    )
                else:
                    self.logger.debug(
                        f"{env}->{zone}: failed to delete: {self.remote_path(env, zone)!s}"
                    )

            with Timeout(30, throwing=False) as timeout:
                _ = driver.send(path_from=self.path, path_to=self.remote_path(env, zone))
                if not _.ok:
                    _msg = f"{env}->{zone}: failed to send: {_.stderr}"
                    self.logger.critical(_msg)
                    raise ExperimentRuntimeError(_msg)
                else:
                    self.logger.info(f"{env}->{zone}: experiment sent!")

            if timeout.timed_out:
                driver.disconnect()

                raise ExperimentRuntimeError("Timed out after 30s during experiment sending")
            ...
            for run in runs:
                ...
                for zone, driver in self.drivers[env].items():
                    fetch_result = driver.fetch(
                        path_from=self.remote_path(env, zone), path_to=self.path
                    )
                    if not fetch_result.ok:
                        _msg = f"{env}->{zone}: failed to fetch logs: {fetch_result.stderr}"
                        self.logger.critical(_msg)
                    else:
                        self.logger.info(f"{env}->{zone}: experiment logs fetched!")
        except (KeyboardInterrupt, SystemExit) as e:
            self.logger.critical(f"{env}: execution interrupted: {type(e)}")
            raise
        except Exception as e:
            self.logger.critical(f"{env}: exception: {e}")
            raise
        finally:
            for zone, driver in self.drivers[env].items():
...
class Run(
    ...
    @property
    def executed_repetitions(self) -> dict:
        return self._executed_repetitions
    ...
    def _bootstrap_apps(self, env: str, rep: int) -> t.Mapping:
        ...
        """
        # env_config = self.parent.environments[env]
        # exp_env_config = self.parent.config.ENVIRONMENTS[env]
        # apps = self.parent.config.ENVIRONMENTS[env].APPS.copy()
        ...
        for app in apps:
            ...
            if apps[app].standalone:
                ...
                apps[app].args += [
                    "1>{}".format(
                        str(
                            self.remote_env_path(env, apps[app].zone)
                            / generic_log_formatter(rep, app, dbg=False)
                        )
                    ),
                    "2>{}".format(
                        str(
                            self.remote_env_path(env, apps[app].zone)
                            / generic_log_formatter(rep, app, dbg=True)
                        )
                    ),
                ]
            ...
            apps[app].app_config["logging"]["app_log_filename"] = str(
                self.remote_env_path(env, apps[app].zone)
                / app_log_formatter(rep, app, dbg=False)
            )
            apps[app].app_config["logging"]["debug_log_filename"] = str(
                self.remote_env_path(env, apps[app].zone)
                / app_log_formatter(rep, app, dbg=True)
            )
            ...
            apps[app].args = ["--json_string", json.dumps(apps[app].app_config.as_dict())]
    ...
    def execute(self, env: str, resume=False) -> None:
        ...
        reps = [
            rep
            for rep, executed in self.executed_repetitions[env].items()
        ...
        for rep in reps:
            ...
            # TODO: refactor process management to the driver!
            master_pids = {zone: [] for zone in self.drivers_proxy}
            master_pgids = {zone: [] for zone in self.drivers_proxy}
            slave_pids = {zone: [] for zone in self.drivers_proxy}
            slave_pgids = {zone: [] for zone in self.drivers_proxy}
            auxiliary_pids = {zone: [] for zone in self.drivers_proxy}
            auxiliary_pgids = {zone: [] for zone in self.drivers_proxy}
            start_individually_pids = {zone: [] for zone in self.drivers_proxy}
            start_threads = []
            ...
            try:
                ...
                for app in [
                    ...
                    pid, _ = self.drivers_proxy[apps[app].zone].spawn(
                        apps[app].executable, apps[app].args
                    )
                    apps[app].pgid = _.pgid
                    apps[app].invoke_result = _
                    slave_pgids[apps[app].zone].append(_.pgid)
                    slave_pids[apps[app].zone] += _.children if _.children else [_.pid]
                    if _.exited is not None:
                        self.logger.error(
                            f"{app!r} executable exited prematurely ({_.exited}), stderr: {_.stderr}"
                        )
                        raise ExperimentRuntimeError(f"{app!r} executable exited prematurely")
                    else:
                        self.logger.debug(
                            f"spawned a {app!r} executable, pgid: {_.pgid}, children: {_.children}"
                        )

                # 2. Launch the src app and add other apps as slaves
                pid, _ = self.drivers_proxy[apps["src"].zone].spawn(
                    apps["src"].executable,
                    apps["src"].args,
                    slave_pids=slave_pids[apps["src"].zone],
                )
                apps["src"].pgid = _.pgid
                apps["src"].pid = _.pid
                apps["src"].invoke_result = _
                master_pgids[apps["src"].zone].append(_.pgid)
                master_pids[apps["src"].zone] += _.children if _.children else [_.pid]
                if _.exited is not None:
                    self.logger.error(
                        f"'src' executable exited prematurely ({_.exited}), stderr: {_.stderr}"
                    )
                    raise ExperimentRuntimeError(f"'src' executable exited prematurely")
                else:
                    self.logger.debug(
                        f"spawned a 'src' executable, pgid: {_.pgid}, children: {_.children}, "
                        f"slave_pids: {slave_pids[apps['src'].zone]}"
                    )

                # 2.1. Delay between spawn & start
                sleep_amount = (
                    1.0
                ...
                # 2.2. Start auxiliary apps with start_individually == True
                for app in [_ for _ in apps if apps[_].get("start_individually", False)]:
                    ...
                    auxiliary_pgids[apps[app].zone].append(_.pgid)
                    auxiliary_pids[apps[app].zone] += _.children if _.children else [_.pid]

                    if _.exited is not None:
                        self.logger.error(
                            f"auxiliary executable {app!r} exited prematurely ({_.exited}), "
                            f"stderr: {_.stderr}"
                        )
                        raise ExperimentRuntimeError(f"{app!r} executable exited prematurely")
                    else:
                        self.logger.debug(
                            f"spawned an auxiliary executable {app!r}, pgid: {_.pgid}, "
                            f"children: {_.children}"
                        )
                ...
                # 3. Start apps
                pids = {zone: [] for zone in self.drivers_proxy}
                start_before_pids = {zone: [] for zone in self.drivers_proxy}

                for zone in pids:
                    pids[zone] += slave_pids[zone] + master_pids[zone]
                    # filder pids that are not meant to be started with the starter below
                    pids[zone] = [
                        _ for _ in pids[zone] if _ not in start_individually_pids[zone]
                    ]

                    start_before_pids[zone] = [
                        _ for _ in slave_pids[zone] if _ not in start_individually_pids
                    ]

                    if not pids[zone]:
                        self.logger.critical(f"no pids to start in zone {zone!r}")
                        raise ExperimentRuntimeError(f"no pids to start in zone {zone!r}")
                ...
                if stagger:
                    ...
                    def starter(_zone, _driver, _master_pids, _start_before_pids, _barrier):
                        self.logger.debug(f"waiting on barrier for zone {_zone}")
                        _barrier.wait()
                        if _start_before_pids:
                            _driver.start(*_start_before_pids)
                        if _master_pids:
                            _driver.start(*_master_pids)

                    for zone, driver in self.drivers_proxy.items():
                        self.logger.debug(
                            f"will start staggered in zone {zone!r}, "
                            f"first pids: {start_before_pids[zone]!r}, "
                            f"then pids: {master_pids[zone]!r}"
                        )
                    ...
                    def starter(_zone, _driver, _pids, _barrier):
                        self.logger.debug(f"waiting on barrier for zone {_zone}")
                        _barrier.wait()
                        if _pids:
                            _driver.start(*_pids)
                    ...
                    for zone, driver in self.drivers_proxy.items():
                        self.logger.debug(
                            f"will start simultaneously in zone {zone!r}, pids: {pids[zone]!r}"
                        )

                        start_threads.append(
                            threading.Thread(
                                target=starter, args=(zone, driver, pids[zone], barrier)
                            )
                        )
                ...
                for zone in pids:
                    self.logger.debug(f"started pids: {pids[zone]} in zone: {zone}")
                        ...
                        while True:
                            ...
                            for zone, driver in self.drivers_proxy.items():
                                if not driver.backend.connected:
                                    ...
                                    driver.backend.connect()
                            apps["src"].invoke_result.update()
                            if apps["src"].invoke_result.exited is not None:
                                break
                            else:
                ...
                # 5.1. Check exit status of 'src' app, stop & kill.
                apps["src"].invoke_result.update()

                if timeout.timed_out or apps["src"].invoke_result.exited is None:
                    self.drivers_proxy[apps["src"].zone].stop(apps["src"].pgid, pgid=True)
                    sleep(0.2)
                    self.drivers_proxy[apps["src"].zone].kill(apps["src"].pgid, pgid=True)
                    self.logger.error(
                        f"'src' executable timed out after {timeout_duration} seconds, was terminated"
                    )

                if apps["src"].invoke_result.exited != 0:
                    self.logger.error(
                        f"'src' executable exited with non-zero return code "
                        f"({apps['src'].invoke_result.exited}), "
                        f"stderr: {apps['src'].invoke_result.stderr}"
                    )
                else:
                    self.logger.debug("'src' executable exited successfully!")

                # 5.2. Check exit statuses and explicitly kill apps
                for app in [_ for _ in apps if _ != "src"]:
                    apps[app].invoke_result.update()
                    if apps[app].invoke_result.exited != 0:
                        self.logger.error(
                            f"app {app!r} executable exited with non-zero return code "
                            f"({apps[app].invoke_result.exited}), "
                            f"stderr: {apps[app].invoke_result.stderr}"
                        )
                    else:
                        self.logger.debug(f"{app!r} executable exited successfully!")
                        break

                    if apps[app].invoke_result.exited is None:
                        self.logger.warning(f"app {app!r} did not exit gracefully!")

                    child_processes = apps[app].invoke_result.children
                    if child_processes is not None:
                        self.logger.debug(f"shutting down child processes {child_processes!r}")
                        self.drivers_proxy[apps[app].zone].stop(*child_processes)
                    else:
                        self.logger.debug(f"shutting down process group {apps[app].pgid}")
                        self.drivers_proxy[apps[app].zone].stop(apps[app].pgid, pgid=True)

                self._executed_repetitions[env][rep] = True
                ...
                for zone, driver in self.drivers_proxy.items():
                    all_pids = [*master_pids[zone], *slave_pids[zone], *auxiliary_pids[zone]]
                    all_pgids = [
                        *master_pgids[zone],
                        *slave_pgids[zone],
                        *auxiliary_pgids[zone],
                    ]

                    self.logger.debug(f"all_pids: {all_pids!r}, all_pgids: {all_pgids!r}")

                    driver.kill(*all_pids)
                    driver.kill(*all_pgids, pgid=True)
