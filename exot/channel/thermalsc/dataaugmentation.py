...
class DataAugmentation(
    ...
    def __init__(self, *args, **kwargs):
        ...
        for matcher_name in ["matcher_data", "matcher_labels"]:
            ...
            assert all(isinstance(n, str) for n in getattr(self.config.DATASET, matcher_name)["values"])
            assert isinstance(getattr(self.config.DATASET, matcher_name)["dimensions"], list)
            assert all(isinstance(n, int) for n in getattr(self.config.DATASET, matcher_name)["dimensions"])
        ...
        # TODO verify label mapping structure
    ...
    def duration_context_change(self):
        return math.ceil(self.config.DATASET.duration_context_change_s / self.resampling_period_s)
    ...
    def _populate_columns_idxes(self):
        ...
        self._label_columns_idxes = list(range(dummy_data[self.matcher_data].shape[1],
                                               dummy_data[self.matcher_data].shape[1]+dummy_data[self.matcher_labels].shape[1]-1))
    ...
    def matcher_data(self):
        if not hasattr(self, '_matcher_data'):
            self._matcher_data = Matcher(self.config.DATASET.matcher_data["quantity"],
    ...
    def matcher_labels(self):
        if not hasattr(self, '_matcher_labels'):
            self._matcher_labels = Matcher(self.config.DATASET.matcher_labels["quantity"],
    ...
    def label_refactoring_rule(self, analysis_phase: str = ''):
        ...
        if hasattr(self.config.DATASET.label_refactoring_rule, 'remove_unknown'):
            if getattr(self.config.DATASET.label_refactoring_rule.remove_unknown, analysis_phase, False):
    ...
    def ingest_args(self, rep: int, analysis_phase: str) -> dict:
        return dict(
                ...
                io={
                    "rep": rep,
                    "matcher":[(self.matcher_data, None),
                               (self.matcher_labels, None)],
                    "synchronise":False,
                   },
                rdp={
                     "mapping":(self.matcher_labels,self.label_mapping),
                    },
                lne={
    ...
    def train_Y_shape(self):
        return (self.num_train_batches, self.batch_size_timesteps, len(self.label_columns_idxes))
    ...
    def _execute_handler(self, *args, **kwargs):
        ...
        get_root_logger().info("Finished preparation of the train, test and verification samples")
    ...
    def _prepare_with_augmentation(self, analysis_phase: str):
        ...
        # --------------------------------------------------------------------------------------------------
        get_root_logger().info("Starting data preparation with augmentation for phase %s" % analysis_phase)
            ...
            while remaining_len >= self.min_len_profile:
                ...
                if cur_run.i_symstream.shape[0] > self.min_len_profile:
                    if batch_idx < self.batch_size and analysis_phase != 'verif':
                        ...
                        max_snippet_len = random.choice([(1+batch_idx)*2*self.min_len_profile, remaining_len, cur_run.i_symstream.shape[0]])
                        min_snippet_len = self.min_len_profile
                        cur_snippet_len = random.randint(min_snippet_len, min([max_snippet_len, remaining_len, cur_run.i_symstream.shape[0]]))
                    else:
                ...
                context_changes = np.where(np.diff(cur_run.i_symstream[cur_snippet_len:, self.label_columns_idxes]) != 0)[0] + cur_snippet_len
                ...
                if len(context_changes) > 0:
                    ...
                    cur_profile_len = cur_snippet_len + (context_changes[0]+self.duration_context_change - context_changes[0])
                    trace = np.empty((cur_profile_len, len(self.data_columns_idxes)))
                    trace[:cur_snippet_len, :] = cur_run.i_symstream[:cur_snippet_len,self.data_columns_idxes]
    ...
    def _prepare_without_augmentation(self, analysis_phase: str):
        """
        """
        get_root_logger().info("Starting data preparation without augmentation for phase %s" % analysis_phase)
        cnt_batch = 0
        ...
        reshape_X        = list(getattr(self,     analysis_phase + '_X_shape')); reshape_X[0] = 0
        reshape_Y        = list(getattr(self,     analysis_phase + '_Y_shape')); reshape_Y[0] = 0
        ...
        if analysis_phase == "verif":
            ...
            shuffled_idxes = np.array(
                shuffle(list(range(getattr(self, 'num_' + analysis_phase + '_batches'))), random_state=random.randint(0, len(getattr(self, analysis_phase + '_runs'))-1)),
                dtype=int)
            setattr(self, analysis_phase + '_X'               , X[shuffled_idxes])
            setattr(self, analysis_phase + '_Y'               , Y[shuffled_idxes].astype(int))
            setattr(self, analysis_phase + '_actual_batch_len', actual_batch_len[shuffled_idxes,:])
    ...
    def compute_weights(self, histlabels, mode=WeightCalculationMode.Equal):
        ...
        if mode == WeightCalculationMode.Proportional:
            ...
            for label in self.label_mapping:
                # TODO hackish
                weights[self.label_mapping[label]['int']] = 1 if label != 'UNKNOWN' and self.label_mapping[label]['str'] != 'HO' else 0.5
            return weights
