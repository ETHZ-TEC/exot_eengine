...
def run_path_unformatter(path: str) -> t.Tuple:
    ...
    def visitor(x):
        ...
        try:
            ...
            assert isinstance(x, t.Hashable), "each component must be hashable"
            return x

    return tuple(visitor(x) for x in split)


def app_log_formatter(rep: int, app: str, dbg: bool) -> Path:
    """Produce a formatted log filename"""

    return "{:>03d}_{}.{}".format(rep, app, "debug.txt" if dbg else "log.csv")


def generic_log_formatter(rep: int, app: str, dbg: bool) -> Path:
    """Produce a formatted log filename"""

    return "{:>03d}_{}.{}".format(rep, app, "stderr.txt" if dbg else "stdout.txt")


def app_log_unformatter(path: Path) -> (int, str, bool):
    """Produce a formatted log filename"""

    _split_by_dot = path.name.split(".")
    assert len(_split_by_dot) == 3, "expect 3 comma-separated components"
    _split_by_underscore = _split_by_dot[0].split("_")
    assert len(_split_by_underscore) == 2, "expect 2 underscode-separated components"

    rep = int(_split_by_underscore[0])
    app = _split_by_underscore[1]
...
class Matcher:
    ...
    def validate(self, df: t.Union[pd.DataFrame, pd.Series, pd.Panel]) -> t.NoReturn:
        ...
        try:
            ...
            raise MatchNotFound(repr(self), e.args)
